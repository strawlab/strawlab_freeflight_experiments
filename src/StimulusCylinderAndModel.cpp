...
public:
    ...
    std::string name() const { return "StimulusCylinderAndModel"; }
...
private:
    ...
    osg::ref_ptr<osg::PositionAttitudeTransform> _model_pat;
    double                              _t0;
    ...
    osg::Vec3                           _model_position;
    osg::Vec3                           _model_scale;
    osg::Quat                           _model_attitude;

    osg::Uniform* tex_v_offset_uniform;

    osg::ref_ptr<osg::Group> create_virtual_world();
    void dirty_cylinder(void);
    void update_model_pat(void);
    void load_model(std::string osg_filename);
...
StimulusCylinderAndModel::StimulusCylinderAndModel(std::string package_share_dir) :
    ...
    _mem("StimulusCylinderAndModel", sizeof(StimulusCylinderSharedStateType),
         Poco::SharedMemory::AccessMode(Poco::SharedMemory::AM_WRITE | Poco::SharedMemory::AM_READ)),
    _memlock("StimulusCylinderAndModel"),
    _model_position(0.,0.,0.),
    _model_scale(1.0,1.0,1.0),
    _model_attitude(0.0,0.0,0.0,1.0)
{

}

void StimulusCylinderAndModel::post_init(bool slave)
{
    _slave = slave;
    _virtual_world = create_virtual_world();

    // when we first start, don't load any model, but create a node that is later deleted.
    _model_pat = new osg::PositionAttitudeTransform;
    update_model_pat();
...
std::vector<std::string> StimulusCylinderAndModel::get_topic_names() const
{
...
void StimulusCylinderAndModel::receive_json_message(const std::string& topic_name, const std::string& json_message)
{
    json_t *root;
    json_error_t error;
...
std::string StimulusCylinderAndModel::get_message_type(const std::string& topic_name) const
{
    std::string result;
...
void StimulusCylinderAndModel::update( const double& time, const osg::Vec3& observer_position, const osg::Quat& observer_orientation )
{
  Poco::NamedMutex::ScopedLock lock(_memlock);
...
osg::ref_ptr<osg::Group> StimulusCylinderAndModel::create_virtual_world() {
...
void StimulusCylinderAndModel::set_cylinder_rotation(float angle) {
    _angular_position_mode = true;
    _angular_position = angle;
}

void StimulusCylinderAndModel::set_cylinder_rotation_rate(float rate) {
    _angular_position_mode = false;
    _angular_velocity = rate;
}

void StimulusCylinderAndModel::set_cylinder_v_offset(float vo) {
    _v_offset_value_mode = true;
    _v_offset_value = vo;
}

void StimulusCylinderAndModel::set_cylinder_v_offset_rate(float vor) {
    _v_offset_value_mode = false;
    _v_offset_rate = vor;
}

void StimulusCylinderAndModel::set_cylinder_position(float x, float y, float z) {
    _cylinder->setCenter(osg::Vec3(x,y,z+(0.5*DEFAULT_HEIGHT)));
    dirty_cylinder();
}

void StimulusCylinderAndModel::set_cylinder_radius(float r) {
    _cylinder->setRadius(r);
    dirty_cylinder();
}

void StimulusCylinderAndModel::set_cylinder_height(float h) {
    _cylinder->setHeight(h);
    dirty_cylinder();
}

void StimulusCylinderAndModel::dirty_cylinder(void) {
    _shape->dirtyDisplayList();
    _shape->dirtyBound();
}

void StimulusCylinderAndModel::update_model_pat(void) {
    _model_pat->setPosition( _model_position );
    _model_pat->setScale( _model_scale );
    _model_pat->setAttitude( _model_attitude );
}

void StimulusCylinderAndModel::load_model( std::string osg_filename ) {
    _virtual_world->removeChild(_model_pat);
    _model_pat = new osg::PositionAttitudeTransform;
    update_model_pat();
    if (osg_filename!="/dev/null") {
        osg::Node* tmp = load_osg_file(osg_filename);
        if (tmp!=NULL) {
            _model_pat->addChild( tmp );
        } else {
