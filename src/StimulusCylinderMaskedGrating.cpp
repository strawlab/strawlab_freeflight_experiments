...
MaskedGratingParams parse_masked_grating_info(const json_t * const root) {
    MaskedGratingParams result;
    ...
    data_json = json_object_get(root, "mask_position");
    flyvr_assert(data_json != NULL);
    flyvr_assert(json_is_real(data_json));
    result.mask_position = json_real_value( data_json );

    data_json = json_object_get(root, "mask_width");
    flyvr_assert(data_json != NULL);
    flyvr_assert(json_is_real(data_json));
    result.mask_width = json_real_value( data_json );

    data_json = json_object_get(root, "mask_intensity");
    flyvr_assert(data_json != NULL);
    flyvr_assert(json_is_real(data_json));
    result.mask_intensity = json_real_value( data_json );
...
typedef struct
{
    ...
    osg::ref_ptr<osg::Uniform>          u_mask_position;
    osg::ref_ptr<osg::Uniform>          u_mask_width;
    osg::ref_ptr<osg::Uniform>          u_mask_intensity;
} MaskedGratingType;
...
typedef struct
{
    ...
    MaskedGratingType gratings[NUM_GRATINGS];
} CylInfo;

class StimulusCylinderMaskedGrating: public StimulusInterface
{
public:
    StimulusCylinderMaskedGrating();
    ...
    std::string name() const { return "StimulusCylinderMaskedGrating"; }
...
private:
    ...
    void set_masked_grating_info( int i, MaskedGratingParams& new_values);
};

StimulusCylinderMaskedGrating::StimulusCylinderMaskedGrating() :
    _t0(-1)
{
    _virtual_world = create_virtual_world();
}

std::vector<std::string> StimulusCylinderMaskedGrating::get_topic_names() const
{
    std::vector<std::string> result;
	result.push_back("masked_grating_info");
    return result;
}

void StimulusCylinderMaskedGrating::receive_json_message(const std::string& topic_name, const std::string& json_message)
{
    json_t *root;
    json_error_t error;
    ...
    if (topic_name=="masked_grating_info") {
        MaskedGratingParams new_values;

        new_values = parse_masked_grating_info(root);
        set_masked_grating_info(0,new_values);
...
std::string StimulusCylinderMaskedGrating::get_message_type(const std::string& topic_name) const
{
    std::string result;
    if (topic_name=="masked_grating_info") {
        result = "strawlab_freeflight_experiments/CylinderMaskedGratingInfo";
    } else {
        throw std::runtime_error("unknown topic name");
    }
    return result;
...
void StimulusCylinderMaskedGrating::update( const double& time, const osg::Vec3& observer_position, const osg::Quat& observer_orientation )
{
    double dt=0.0;
    if (_t0 > 0) {
    ...
    for (int i=0; i<NUM_GRATINGS; i++) {
        MaskedGratingType &grating = _cyl.gratings[i];
        {
            ...
            if (grating.dirty) {
                ...
                grating.u_mask_position->set(grating.params.mask_position);
                grating.u_mask_width->set(grating.params.mask_width);
                grating.u_mask_intensity->set(grating.params.mask_intensity);
...
void StimulusCylinderMaskedGrating::init_cyl(CylInfo& cyl) {
    ...
    const char* mask_position_name;
    const char* mask_width_name;
    const char* mask_intensity_name;
    ...
    for (int i=0; i<NUM_GRATINGS; i++) {
        ...
        new_values.mask_position = 0;
        new_values.mask_width = 20*D2R;
        new_values.mask_intensity = 0.5;
        ...
        if (i==0) {
            ...
            mask_position_name="mask_position0";
            mask_width_name="mask_width0";
            mask_intensity_name="mask_intensity0";
        } else {
            flyvr_assert(false);
        }
        ...
        cyl.gratings[i].u_mask_position = new osg::Uniform( osg::Uniform::FLOAT, mask_position_name );
        cyl.gratings[i].u_mask_width = new osg::Uniform( osg::Uniform::FLOAT, mask_width_name );
        cyl.gratings[i].u_mask_intensity = new osg::Uniform( osg::Uniform::FLOAT, mask_intensity_name );

        set_masked_grating_info( i, new_values);
    }
    ...
    load_shader_source( cyl.vertex_shader, "masked_grating_blended.vert" );
    load_shader_source( cyl.fragment_shader, "masked_grating_blended.frag" );
    ...
    for (int i=0;i<NUM_GRATINGS;i++) {
        ...
        cyl.state_set->addUniform( cyl.gratings[i].u_mask_position );
        cyl.state_set->addUniform( cyl.gratings[i].u_mask_width );
        cyl.state_set->addUniform( cyl.gratings[i].u_mask_intensity );
    }

}

osg::ref_ptr<osg::Group> StimulusCylinderMaskedGrating::create_virtual_world() {
...
void StimulusCylinderMaskedGrating::post_init(bool slave)
{
...
void StimulusCylinderMaskedGrating::set_masked_grating_info(int i, MaskedGratingParams &new_values) {
    MaskedGratingType &grating = _cyl.gratings[i];
    {
        OpenThreads::ScopedLock<OpenThreads::Mutex> lock(grating.mutex);
...
POCO_EXPORT_CLASS(StimulusCylinderMaskedGrating)
POCO_END_MANIFEST
