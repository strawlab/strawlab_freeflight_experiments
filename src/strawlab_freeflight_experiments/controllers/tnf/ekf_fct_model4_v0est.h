...
typedef struct {
    ...
    double Qn[16]; // covariance matrix process noise
    double P0[16]; // start value for covariance matrix of estimation error
    double x0[4]; // estimated initial state of system
    double Ts; // sampling time
    flyparams_t ekf_flyparams;
}ekfp_t;

typedef struct {
    double xminus[4];  // estimated state in the EKF
    double Pminus[16]; // covariance matrix of estimation error
    double xest[4];    // provided estimated state, is equivalent to xminus, 
