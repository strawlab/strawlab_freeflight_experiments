...
int decFct (double *xpos, double *ypos, int *id, int arrayLen, int reset,
     ...
     * contrState: struct containing all the internal variables and status of the controller
     * ekfState: struct holding the state of the EKF with xminus, Pminus, xest, and status
	 * gammaEstimate: contains an estimate of gamma (orientation of the fly)
	...
	// specific for path of lemniscate
	double R = cp->R;
    ...
    for (i=0;i<arrayLen;i++) {
		...
		if (cp->path_type==1) {
			// ellipse
			// if the fly is inside the ellipse 
			// the distance is negative, otherwise positive: 
			...
			//printf("closest point: %g %g\n",closestPoint[0],closestPoint[1]);
			
			// Crop value of closestPoint[0] to the interval [-a,a]
			// (due to numerical inaccuracies it might be that it is not 
			// contained in the interval)
			if (closestPoint[0] < -a) {
				printf("cropped: %g\n",closestPoint[0]+a);
				closestPoint[0] = -a;
			} else if (closestPoint[0] > a) {
				printf("cropped: %g\n",closestPoint[0]-a);
				closestPoint[0] = a;
			}
			...
			// lemniscate
			// the calculated distance is always positive
			sd[i] = distancePointToLemniscate (x,y,thetaMin+i,R);
	...
	printf("thetaMin: %g\n",thetaMin[indexClosestFly]*180/M_PI);
	printf("sd: %g\n",distClosestFly);
    ...
    if (distClosestFly <= THRES_DIST) {
        ...
        //       => good for initializing zeta1 in the controller
        
        // set initial condition for zeta1 of controller to theta closest to 
        // the current fly position
        cp->zeta0[0] = thetaMin[indexClosestFly]; 
        // reset controller
        initController (contrState, cp);
		...
		// furthermore use the estimate for gamma
        (ekfp->x0)[0] = xpos[indexClosestFly]; 
        (ekfp->x0)[1] = ypos[indexClosestFly]; 
		(ekfp->x0)[2] = gammaEstimate[0];
        // reset EKF
        initEKF (ekfState, ekfp);
...
double distancePointToLemniscate (double x,double y, double *thetaMin, double R) {
	/* Calculate the minimum distance of a point with coordinates x and y
	 * to a lemniscate with geometric parameter R. The corresponding value of the 
	 * path parameter for the closest point on the lemniscate is returned as thetaMin[0]. 
	 */
	
	// Brute-force method, the path parameter is discretized, all corresponding
	// point on the lemniscate are checked whether they are closest to (x,y). 
	
	int i;
	
	int Nd = 500; // number of discretization points
	double distMin = 1e9; // minimum distance of any point on the lemniscate to (x,y) so far
	double curTheta = 0.0;
	double deltaTheta = 2*M_PI/((double)Nd);
	double py[2];
	double curDist;
	
	for (i=0;i<Nd;i++) {
		pathLemniscateData_easy (py, curTheta, R);
		curDist = sqrt(pow(x-py[0],2) + pow(y-py[1],2));
		if (curDist < distMin) {
			distMin = curDist;
			thetaMin[0] = curTheta;
		}		
