...
def read_preprocess_cache_1(uuids=DCN_COMPLETED_EXPERIMENTS,
    ...
    provenance = [FreeflightAnalysisFiles(op.join(CACHE_DIR, uuids[0])).configuration().id()]  # All same...
    ...
    if not op.isfile(cache_file) or recompute:
        ...
        for transformer in transformers:
            ...
            provenance.append(transformer.configuration().id())
        ...
        provenance.extend([contract.configuration().id() for contract in CONTRACTS])
        print '\tChecked:\n\t\t%s' % '\n\t\t'.join(checked)
        ...
        for series_extractor in series_extractors:
            print '\t\t' + series_extractor.configuration().id()
            series_extractor.compute(trajs)
            provenance.append(series_extractor.configuration().id())
