...
import analysislib.series as afeat
...
class TestFeaturesNumeric(unittest.TestCase):
    ...
    def test_velocity(self):
        ...
        m = afeat.MultiSeriesComputer('vx', 'vy', 'vz', 'velocity')
        m.process(df, dt)
    ...
    def test_dtheta(self):
        ...
        m = afeat.MultiSeriesComputer('dtheta')
        m.process(df, dt)
...
class TestFeaturesAPI(unittest.TestCase):
    ...
    def test_set(self):
        m = afeat.MultiSeriesComputer('vx')
        self.assertEqual(repr(m.series), "[Measurement(col='x'), VxFeature(col='vx')]")
        self.assertEqual(len(m.series), 2)

        m.set_series()
        self.assertListEqual(m.series, [])
    ...
    def test_add(self):
        ...
        m.add_series_by_column_added('vx')
        self.assertEqual(repr(m.series), "[Measurement(col='x'), VxFeature(col='vx')]")
        self.assertEqual(len(m.series), 2)
        self.assertTupleEqual(m.get_columns_added(), ('vx',))

        self.assertRaises(ValueError, m.add_series_by_column_added, 'NO_SUCH_COLUMN')
        self.assertRaises(ValueError, m.add_series, 'NO_SUCH_FEATURE')

    def test_more(self):
        m = afeat.MultiSeriesComputer('dtheta')
        self.assertEqual(len(m.series), 6)
        self.assertTupleEqual(m.get_columns_added(), ('vx', 'vy', 'theta', 'dtheta'))

        #check the order of resolution is correct
        cls = afeat.get_series_class('dtheta')
        self.assertTrue(isinstance(m.series[-1], cls))
    ...
    def test_what(self):
        cls = afeat.get_series_class('rcurve')
    ...
    def test_api(self):
        f = afeat.get_series_class('rcurve')()
        self.assertTupleEqual(f.get_depends(),('x','y'))

        f = afeat.get_series_class('vx')()
    ...
    def test_lazy_compute(self):
        ...
        m = afeat.MultiSeriesComputer('vx', 'ratiouw')
...
class TestSomeFeatures(unittest.TestCase):
    ...
    def test_all_features_and_deps(self):
        for name in afeat.ALL_SERIES_NAMES:
            ...
            m = afeat.MultiSeriesComputer(name)
            df = self._df.copy()
