...
SWITCH_MODE_TIME    = 300    #alternate between control and static (i.e. experimental control) seconds
...
# mix conditions
shuffle(CONDITIONS_raw)
breakCond = "infinity04.svg/0/0.00/0/1000.0/-0.01"
# add break
CONDITIONS_withBreak = [[cond,cond,breakCond] for cond in CONDITIONS_raw]
#flatten List
CONDITIONS = [item for sublist in CONDITIONS_withBreak for item in sublist]
for i in CONDITIONS:
    print i
...
class Node(object):
    ...
    def switch_conditions(self,event,force=''):
        if force:
            ...
            self.conditionID = self.conditionID +1
            if self.conditionID >= len(CONDITIONS):  self.conditionID = 0
            j = self.conditionID
            print self.conditionID
