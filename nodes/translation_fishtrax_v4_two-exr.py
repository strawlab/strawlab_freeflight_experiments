...
FLY_DIST_MIN_DIST   = 0.001    # minimum distance fly must move in above interval to not be ignored
...
class Node(nodelib.node.Experiment):
    def __init__(self, args):
        super(Node, self).__init__(args=args,
                                   state=("stimulus_filename","trg_x","trg_y","trg_z","cyl_x","cyl_y","cyl_r","ratio","stim_x","stim_y","stim_z"))
	...
	print '#'*20, String(self.condition.get("geom_json_buf", ""))        
        ...
        rospy.loginfo('-> %s condition: %s osg=%s (p=%.1f, svg=%s, advance=%.1fpx)' % (self.condition_switch_count,self.condition,stimulus_filename,self.p_const,os.path.basename(self.svg_fn),self.advance_px))
