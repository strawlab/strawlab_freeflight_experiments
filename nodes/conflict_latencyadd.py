...
# like the conflict node, but only applies the lag once the
# fly gets to Z_TARGET +/- 0.1m

class Node(nodelib.node.Experiment):
    def __init__(self, args):
        super(Node, self).__init__(args=args,
                                   state=("rotation_rate","trg_x","trg_y","trg_z","cyl_x","cyl_y","cyl_r","ratio","v_offset_rate","phase","model_x","model_y","model_z","model_filename"))
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if currently_locked_obj_id is None:
                ...
                # wait until the fly gets low before adding the lag
                if not self.fly_is_low and (abs(fly_z - self.z_target) < 0.1):
                    self.fly_is_low = True
                    self.pub_lag.publish(self.lag)

                # for debugging
                active = self.fly_is_low
                ...
                # we don't do any Z-control once the fly has gone low
                if self.fly_is_low:
                    v_rate = 0.0
                else:
