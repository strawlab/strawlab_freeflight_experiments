...
class Node(nodelib.node.Experiment):
    def __init__(self, args):
        super(Node, self).__init__(args=args,
                                   state=("rotation_rate", "trg_x","trg_y","trg_z","cyl_x","cyl_y","cyl_r","ratio","stim_x","stim_y","stim_z", "ratio","perturb_progress"))
    ...
    def switch_conditions(self):
        ...
        self.perturber = sfe_perturb.NoPerturb() # no perturbation by default
        ...
        self.translation_gain = float(self.condition['star_translation_gain'])
    ...
    def get_starfield_translation_vector(self, fly_x, fly_y, fly_z, fly_vx, fly_vy, fly_vz, now, framenumber, currently_locked_obj_id):
        ...
        return self.translation_gain*dx,self.translation_gain*dy,self.trg_x,self.trg_y
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if currently_locked_obj_id is None:
                ...
                # it is possible to give to the star field a translational and a rotational
                # component to force the fly to follow the desired path...
                rate_x,rate_y,trg_x,trg_y = self.get_starfield_translation_vector(fly_x, fly_y, fly_z, fly_vx, fly_vy, fly_vz, now, framenumber, currently_locked_obj_id)
                v_rate = self.get_v_rate(fly_z)
                rate,trg_x,trg_y = self.get_starfield_rotation_vector(fly_x, fly_y, fly_z, fly_vx, fly_vy, fly_vz, now, framenumber, currently_locked_obj_id)
